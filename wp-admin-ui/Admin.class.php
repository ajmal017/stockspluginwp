...
if ( isset( $_GET['action'])){
	if($_GET['action'] == 'init_Companies'){
		$class = new WP_Admin_UI;
		$class->init_Companies();
			...
			<div id ="AdminControls">
				<div id = "init_Companies"> 
					<input type="button" value="Initialize companies" class="button" 
						onclick="document.location='<?php echo $this->var_update(array('action'=>'init_Companies')); ?>';" />
				</div>
...
<?php
	...
	function init_Companies ()
    {	
        global $wpdb;
		$sql = "SELECT count(*) AS Count
				FROM information_schema.TABLES
				WHERE (TABLE_SCHEMA = 'wordpress') AND (TABLE_NAME = 'wp_nepse_stocks')";
		$result = $wpdb->get_results($sql);
		
		
		if ($result[0]->Count == 0){
			$createTable_stocks = "CREATE TABLE `wp_nepse_stocks` (
									  `id` int(11) NOT NULL,
									  ...
									  `tradedShares` int(11) NOT NULL,
									  `closingPrice` float NOT NULL,
									  `minPrice` float NOT NULL
									) ENGINE=InnoDB DEFAULT CHARSET=latin1;
									
									";
			$wpdb->query($createTable_stocks);
			$stocksAddPrimaryKey = "ALTER TABLE `wp_nepse_stocks`
										ADD PRIMARY KEY (`id`);";
			$wpdb->query($stocksAddPrimaryKey);
			$stocksAddAI= "	ALTER TABLE `wp_nepse_stocks`
								MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;";
			$wpdb->query($stocksAddAI);
		...
		foreach($data as $row){
			...
			if ($result[0]->CompanyName == ''){
					$INSERTStocks = "INSERT INTO wp_nepse_stocks 
							 (`CompanyName`,  `transactions`, 
							  `difference`, `maxPrice`,`tradedShares`,
								`closingPrice`, `minPrice`
							  ) 
							VALUES 
	...
	function UpdateStocksInfo(){
		...
		$sql = "SELECT count(*) AS Count
				FROM information_schema.TABLES
				WHERE (TABLE_SCHEMA = 'wordpress') AND (TABLE_NAME = 'wp_nepse_stocks_info')";
		$result = $wpdb->get_results($sql);
		
		
		if ($result[0]->Count == 0){
			$createTable_stocks = "CREATE TABLE `wp_nepse_stocks_info` (
									  `id` int(11) NOT NULL,
									  `CompanyName` varchar(255) NOT NULL,
									  `CompanySymbol` varchar(255) ,
									  `Sector` varchar(255) ,
									  `logo` varchar(255)
									) ENGINE=InnoDB DEFAULT CHARSET=latin1;
									...
									";
			$wpdb->query($createTable_stocks);
			$stocksAddPrimaryKey = "ALTER TABLE `wp_nepse_stocks_info`
										ADD PRIMARY KEY (`id`);";
			$wpdb->query($stocksAddPrimaryKey);
			$stocksAddAI= "	ALTER TABLE `wp_nepse_stocks_info`
								MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;";
			$wpdb->query($stocksAddAI);
